...
save_outputs_to_file("./plot/test_5_out_stoh.dat", outputs_errors)
save_errors_to_file("./plot/test_5_error_stoh.dat", outputs_errors)

errors_file = File.open("erros_stoh.dat", "w+")
errors_file.write(anfis.errors.each_with_index.map{ |er, i| "#{i * error_iteration} #{er}"}.join("\n"))

params_file = File.open("params_stoh.dat", "w+")
